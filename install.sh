...
sudo apt-get install -y git make cmake curl zip ranger exa autojump nitrogen arandr psmisc dunst libnotify-bin lxappearance qt5ct qt5-style-kvantum nautilus flameshot rxvt-unicode volumeicon-alsa blueman network-manager-gnome neofetch polybar rofi gdm3 > /dev/null ;

# Install dependencies for picom
# https://github.com/ibhagwan/picom
echo "Installing packages for picom" ;
sudo apt-get install -y libxext-dev libxcb1-dev libxcb-damage0-dev libxcb-xfixes0-dev libxcb-shape0-dev libxcb-render-util0-dev libxcb-render0-dev libxcb-randr0-dev libxcb-composite0-dev libxcb-image0-dev libxcb-present-dev libxcb-xinerama0-dev libxcb-glx0-dev libpixman-1-dev libdbus-1-dev libconfig-dev libgl1-mesa-dev libpcre2-dev libpcre3-dev libevdev-dev uthash-dev libev-dev libx11-xcb-dev > /dev/null ;

# Install dependecies for i3-gaps
# https://github.com/Airblader/i3
echo "Installing packages for i3-gaps" ;
sudo apt-get install -y meson dh-autoreconf libxcb-keysyms1-dev libpango1.0-dev libxcb-util0-dev xcb libxcb1-dev libxcb-icccm4-dev libyajl-dev libev-dev libxcb-xkb-dev libxcb-cursor-dev libxkbcommon-dev libxcb-xinerama0-dev libxkbcommon-x11-dev libstartup-notification0-dev libxcb-randr0-dev libxcb-xrm0 libxcb-xrm-dev libxcb-shape0 libxcb-shape0-dev > /dev/null ;
...
git clone https://github.com/Airblader/i3.git i3-gaps > /dev/null && 
cd i3-gaps &&
...
curl -L0 https://download.mozilla.org/?product=firefox-develedition-latest-ssl&os=linux&lang=en-CA --output firefox.tar.bz2 > /dev/null &&
sudo mv firefox.tar.bz2 /opt &&
cd /opt &&
sudo tar xfj firefox.tar.bz2 > /dev/null &&
...
curl -L0 https://github.com/matheuuus/dracula-theme/archive/refs/heads/master.zip --output Dracula.zip > /dev/null &&
...
curl -L0 https://github.com/matheuuus/dracula-icons/archive/refs/heads/main.zip --output Dracula-icons.zip > /dev/null && 
unzip Dracula-icons.zip > /dev/null &&
...
sudo apt-get install -y fonts-ipaexfont-gothic > /dev/null;

# Font Awesome
echo "Installing Font-Awesome font"
curl -L0 https://github.com/FortAwesome/Font-Awesome/archive/refs/heads/master.zip --output Font-Awesome.zip > /dev/null &&
unzip Font-Awesome.zip > /dev/null && 
...
curl -L0 https://github.com/ryanoasis/nerd-fonts/releases/download/v2.1.0/Overpass.zip --output NerdFonts.zip > /dev/null &&
unzip NerdFonts.zip > /dev/null &&
...
git clone https://natperron@github.com/natperron/dotfiles.git > /dev/null &&
